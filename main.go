...
func getTodos(rw http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		rnd.JSON(rw, http.StatusBadRequest, renderer.M{"message": "Could not fetch the todo collection", "error": err.Error()})
	...
	rnd.JSON(rw, http.StatusOK, GetTodoResponse{Message: "All todos retrieved", Data: todoList})
}
